...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        // Récupération de l'utilisateur s'il est présent
        User user = userOptional.get();

        // Récupération des autorités (rôles) de l'utilisateur, ici "USER"
        List<GrantedAuthority> authorities = getGrantedAuthorities("USER");

        // Création et retour d'un objet UserDetails avec l'email, le mot de passe et les autorités
        return new org.springframework.security.core.userdetails.User(
            user.getEmail(), user.getPassword(), authorities);
    }

    // Méthode pour créer la liste des rôles (authorities) de l'utilisateur
